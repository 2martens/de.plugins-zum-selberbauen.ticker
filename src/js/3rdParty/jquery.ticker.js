	...
	jQuery.fn.ticker = function(options) { 
		...
		return this.each(function() { 
			...
			function initialisePage() {
				...
				jQuery(settings.dom.wrapperID).append('<div id="' + settings.dom.tickerID.replace('#', '') + '" class="ticker"><div id="' + settings.dom.titleID.replace('#', '') + '" class="ticker-title"><span><!-- --></span></div><p id="' + settings.dom.contentID.replace('#', '') + '" class="ticker-content"></p><div id="' + settings.dom.revealID.replace('#', '') + '" class="ticker-swipe"><span><!-- --></span></div></div>');
				jQuery(settings.dom.wrapperID).removeClass('no-js').addClass('ticker-wrapper has-js ' + opts.direction);
				// hide the ticker
				jQuery(settings.dom.tickerElem + ',' + settings.dom.contentID).hide();
				// add the controls to the DOM if required
				if (opts.controls) {
					// add controls HTML to DOM
					jQuery(settings.dom.wrapperID).append('<ul id="' + settings.dom.controlsID.replace('#', '') + '" class="ticker-controls"><li id="' + settings.dom.playPauseID.replace('#', '') + '" class="jnt-play-pause controls"><a href=""><!-- --></a></li><li id="' + settings.dom.prevID.replace('#', '') + '" class="jnt-prev controls"><a href=""><!-- --></a></li><li id="' + settings.dom.nextID.replace('#', '') + '" class="jnt-next controls"><a href=""><!-- --></a></li></ul>');
					// add related events - set functions to run on given event
					jQuery(settings.dom.controlsID).on('click mouseover mousedown mouseout mouseup', function (e) {
						...
						else if (e.type == 'mouseover' && jQuery('#' + button).hasClass('controls')) {
							jQuery('#' + button).addClass('over');
						}
						else if (e.type == 'mousedown' && jQuery('#' + button).hasClass('controls')) {
							jQuery('#' + button).addClass('down');
						}
						else if (e.type == 'mouseup' && jQuery('#' + button).hasClass('controls')) {
							jQuery('#' + button).removeClass('down');
						}
						else if (e.type == 'mouseout' && jQuery('#' + button).hasClass('controls')) {
			...
			function setupContentAndTriggerDisplay() {
				...
				distance = jQuery(settings.dom.contentID).width();
			...
			function revealContent() {
				...
				if(settings.play) {	
					...
					var offset = jQuery(settings.dom.titleID).width() + 20;
			...
			function pauseTicker() {				
				...
				jQuery(settings.dom.tickerID + ',' + settings.dom.revealID + ',' + settings.dom.titleID + ',' + settings.dom.titleElem + ',' + settings.dom.revealElem + ',' + settings.dom.contentID).stop(true, true);
